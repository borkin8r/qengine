...
char *NET_BaseAdrToString(netadr_t a)
{
  ...
  case NA_MULTICAST6:
    ...
    if (getnameinfo((struct sockaddr *) &ss, sizeof(struct sockaddr_in6), s, sizeof(s), NULL, 0, NI_NUMERICHOST)) {
...
qboolean NET_GetPacket(netsrc_t sock, netadr_t *net_from, sizebuf_t *net_message)
{
  ...
  for (protocol = 0; protocol < 3; protocol++) {
    ...
    fromlen = sizeof(from);
    ret =
        recvfrom(net_socket, (char *) net_message->data, net_message->maxsize, 0, (struct sockaddr *) &from, &fromlen);
  ...
  case AF_INET6:
    ...
    if (multicast_interface != NULL) {
      ...
      if (type == NS_SERVER) {
        ...
        memcpy(&mreq.ipv6mr_multiaddr.s6_addr, &((struct sockaddr_in6 *) (res->ai_addr))->sin6_addr,
               sizeof(mreq.ipv6mr_multiaddr.s6_addr));
        freeaddrinfo(res);

        Error = setsockopt(newsocket, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char *) &mreq, sizeof(mreq));
...
int NET_IPXSocket(int port)
{
  ...
  /* make it broadcast capable */
  if (setsockopt(newsocket, SOL_SOCKET, SO_BROADCAST, (char *) &t, sizeof(t)) == -1) {
